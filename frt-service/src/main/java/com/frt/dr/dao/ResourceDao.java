...
//			captureParam(cq, cb, root, predicates, params, new String[] {"telecom"}, new Class[] {String.class}, com.frt.dr.model.base.PatientContactPoint.class);
	...
	private <T extends com.frt.dr.model.ResourceComplexType> void captureParam(
			CriteriaQuery cq, 
			CriteriaBuilder cb, 
			Root root, 
		...
		if (refClazz.equals(com.frt.dr.model.base.PatientAddress.class)) {
			Root<com.frt.dr.model.base.PatientAddress> subRoot = subQuery.from(com.frt.dr.model.base.PatientAddress.class);
			...
			for (int i=0; i<paramNames.length; i++) {
				if (paramNames[i].equals("address")) {
					...
					throw new IllegalArgumentException("Invalid parameter: " + paramNames[i] + ", for sub query on : " + refClazz.getCanonicalName());
				}
			}
			...
			Root<com.frt.dr.model.base.PatientIdentifier> subRoot = subQuery.from(com.frt.dr.model.base.PatientIdentifier.class);
			subQuery.select(subRoot.<BigInteger> get("patient"));
			for (int i=0; i<paramNames.length; i++) {
			...
			Root<com.frt.dr.model.base.PatientHumanName> subRoot = subQuery.from(com.frt.dr.model.base.PatientHumanName.class);
			subQuery.select(subRoot.<BigInteger> get("patient"));
			for (int i=0; i<paramNames.length; i++) {
			...
			Root<com.frt.dr.model.base.PatientContactPoint> subRoot = subQuery.from(com.frt.dr.model.base.PatientContactPoint.class);
			subQuery.select(subRoot.<BigInteger> get("patient"));
			for (int i=0; i<paramNames.length; i++) {
	...
	private void captureParam(CriteriaBuilder cb, Root root, List<Predicate> predicates, Map params, String paramName, Class paramType) {
		...
		if (value != null && !value.isEmpty()) {
			...
			if (paramType.equals(Integer.class)
					...
					||paramType.equals(Number.class)
					||paramType.equals(BigInteger.class)
					) {
				if (value.startsWith(PARAM_PREFIX_NE)) {
					predicates.add(cb.notEqual(root.get(paramName), cb.parameter(paramType, paramName)));
