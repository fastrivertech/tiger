...
public class PatientLinkMapper extends BaseMapper {
	private static Logger logger = Logger.getLog(PatientLinkMapper.class.getName());
	private static Localization localizer = Localization.getInstance();

	private Class sourceClz;
	private Class targetClz;

	public PatientLinkMapper() {
	}

	@Override
	public PatientLinkMapper from(Class sourceClz) {
		this.sourceClz = sourceClz;
		return this;
	}

	@Override
	public PatientLinkMapper to(Class targetClz) {
	...
	public Object map(Object source) throws MapperException {
		if (!(source instanceof JsonElement)) {
			throw new IllegalArgumentException("PatientLinkMapper.map(source) expects JsonElement, got source of type: "
